...
export function CashflowCharts({ isLoadingAnalytics, cashflowAnalytics }: CashflowChartsProps) {
  return (
    <div className="space-y-3 mb-4">
      ...
      <div className="glass-card rounded-xl p-4">
        ...
        ) : cashflowAnalytics?.expenses?.length > 0 ? (
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={cashflowAnalytics.expenses}
                  innerRadius={45}
                  outerRadius={65}
                  paddingAngle={5}
                  dataKey="value"
                  nameKey="name"
                >
                  {cashflowAnalytics.expenses.map((entry: any, index: number) => (
      ...
      <div className="glass-card rounded-xl p-4">
        ...
        {isLoadingAnalytics ? (
          <div className="h-48 flex justify-center items-center"><Activity className="w-5 h-5 animate-spin text-slate-400" /></div>
        ) : cashflowAnalytics?.income?.length > 0 ? (
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={cashflowAnalytics.income}>
                ...
                <Tooltip cursor={{fill: '#f8fafc', opacity: 0.4}} formatter={(value: any) => `$${Number(value).toLocaleString()}`} />
